...
async def export_to_spotify_account(token_info: dict, playlist_name: str, tracks: list[VerifiedTrack]) -> str:
    ...
    # 1. Create playlist using POST /v1/me/playlists to avoid 403 Forbidden
    headers = {
    ...
    resp = requests.post("https://api.spotify.com/v1/me/playlists", headers=headers, json=payload)
    if resp.status_code not in (200, 201):
        raise RuntimeError(f"Failed to create Spotify playlist: {resp.status_code} - {resp.text}")
    ...
    # 2. Add tracks via Spotipy's playlist_add_items (which uses /v1/playlists/{playlist_id}/items)
    sp_user = spotipy.Spotify(auth=token_info["access_token"])
    
    # Ensure track URIs are correctly formatted
    track_uris = [
        f"spotify:track:{t.spotify_id}" if not t.spotify_id.startswith("spotify:track:") else t.spotify_id 
        for t in tracks
    ]
